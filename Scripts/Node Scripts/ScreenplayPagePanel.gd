...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		...
		for shotline_node in children:
			if not shotline_node is ShotLine2DContainer:
				continue
...
func _on_shotline_clicked(shotline2D: ShotLine2DContainer, button_index: int) -> void:
...
func _on_shotline_dragged(shotline_node: ShotLine2DContainer) -> void:
